    ...
    return (
        <div className=' w-full h-full flex flex-col gap-6 lg:pt-0 pt-4  ' >
            <div className={` w-full ${tab ? " lg:block hidden " : ""} `} > 
                <PageHeader back={true} path={(member) ? `/dashboard/event/details/bymembers/${id}` : curate ? `/dashboard/message` : index ? `/dashboard/message` : `/dashboard/event/details/${id}`} header="Event Messages" body="View & respond to messages from your supporters" /> 
            </div>
