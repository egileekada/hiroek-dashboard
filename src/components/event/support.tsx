...
export default function EventSupport({ tab, setTab }: IProps) {
    ...
    const { createConversation, loadingConversation, createChat, loadingChat } = useConversation()
    ...
    useEffect(() => {
        ...
        if (condata?.id) {
            ...
            socket.on(`conversation-${condata?.id}`, (message: any) => {
                console.log(message);
    ...
    return supportHookForm(
        <div className=' w-full flex gap-4 h-full' >
            ...
            <div className={` w-full ${(tab && !name) ? " lg:hidden " : tab ? " flex " : " hidden "} flex-col h-full items-end `} >
                <LoadingAnimation width="w-full lg:max-w-[393px] " loading={loadingConversation} >
                    <div className=" w-full lg:max-w-[393px] relative flex flex-col lg:mt-auto h-full lg:h-[60vh] rounded-3xl lg:border-[5px] lg:border-primary " >
                        ...
                        <div className=" w-full flex sticky top-0 flex-col bg-white gap-1 pb-2 h-[20vh] rounded-t-3xl pt-3 lg:px-3 px-4  " >
                            ...
                            <div className=" flex items-center gap-2 " >
                                <div className=" w-10 h-10 rounded-full border-2 border-primary " >
                                    <img src={photo + ""} alt="current" className=" w-full h-full rounded-full object-cover " />
                                </div>
                                <div className=" flex flex-col " >
                                    <Text className=" text-sm font-bold text-primary tracking-[1%] " >{name}</Text>
                                </div>
                            </div>
                        </div>
                        <div className=" w-full h-[60vh] lg:h-[35vh] flex flex-col-reverse overflow-y-auto gap-3 pb-[10px] lg:px-3 px-6 " >
                        ...
                        <div className=" w-full mt-auto lg:block hidden sticky bottom-0 p-3 bg-white rounded-b-3xl " >
                            <CustomInput setValue={changeHandler} value={inputMessage} borderRadius="9999px" placeholder="Type your message..." name="chat" edit={true} type="text" hasIcon={true} icon={
